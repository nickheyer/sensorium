...
certs: clean
	...
	@if [ ! -f internal/pki/embedded/ca.crt ]; then \
		...
		echo "subjectAltName=DNS:localhost,DNS:sensorium,DNS:pulsar,DNS:*.ghostnet.sh,DNS:*.heyer.app,DNS:*.local,DNS:*.localdomain,IP:127.0.0.1,IP:192.168.1.54,IP:192.168.1.32,IP:192.168.1.27" > internal/pki/embedded/broker.ext; \
